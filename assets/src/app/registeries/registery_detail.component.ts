...
export class RegisteryDetailComponent {
  ...
  getRegistery(registery_id): void {
    this.registeryService.getRegistery(registery_id)
    .subscribe(registery => {
      ...
      if(this.registery.params.manifests &&
        ...
        var url = this.registery.params.manifests[0].paths[0].replace("/dash", "/stream")
