...
export class OrdersComponent {
  ...
  source_link(workflow) {
    if (workflow.artifacts.length > 0) {
      ...
      const ttml_path = this.getDestinationFilename(workflow, '.ttml', [
        'synchronised.ttml',
        'file_positioned.ttml',
      ])
  ...
  play_original_version(workflow) {
    if (workflow.artifacts.length > 0) {
      const original_mp4_path = this.getDestinationFilename(workflow, 'mp4', [
        'enhanced.mp4',
      ])
  ...
  play_enhanced_version(workflow) {
    if (workflow.artifacts.length > 0) {
      const enhanced_mp4_path = this.getDestinationFilename(
        workflow,
        'enhanced.mp4',
      )
    ...
    const result = workflow.jobs.filter((job) => {
      if (job.name == 'job_transfer' || job.name == 'job_transfer_optim') {
        ...
        if (parameter.length > 0) {
          ...
          if (notExtension) {
            const notExtensionMatch = notExtension.filter((extension) =>
              sourceFilename.endsWith(extension),
            ).length
