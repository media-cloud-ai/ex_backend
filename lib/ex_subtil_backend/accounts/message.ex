defmodule ExSubtilBackend.Accounts.Message do
  @moduledoc """
  ...
  2. Remove the lib/ex_subtil_backend/mailer.ex file
  ...
  def reset_request(address, nil) do
    prep_mail(address)
    |> subject("[Subtil Backend] Reset your password")
    |> text_body(
  ...
  def reset_success(address) do
    prep_mail(address)
    |> subject("[Subtil Backend] Password reset")
