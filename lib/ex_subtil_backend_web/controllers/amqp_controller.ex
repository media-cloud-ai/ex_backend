defmodule ExSubtilBackendWeb.Amqp.AmqpController do
  ...
  def get_amqp_informations(endpoint) do
    ...
    port =
      System.get_env("AMQP_MANAGEMENT_PORT") || Application.get_env(:amqp, :management_port) || 15672
      |> port_format

    HTTPotion.get("http://" <> hostname <> ":" <> port <> "/api/" <> endpoint, [basic_auth: {username, password}]).body
    |> Poison.decode!
  end
